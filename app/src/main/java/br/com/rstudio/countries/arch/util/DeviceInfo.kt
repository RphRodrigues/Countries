      ...
      get() {
        ...
        return (
          Build.MANUFACTURER.contains("Genymotion")
            ...
            || Build.MODEL.toLowerCase(locale).contains("droid4x")
            || Build.MODEL.contains("Emulator")
            || Build.MODEL.contains("robolectric")
            || Build.MODEL.contains("Android SDK built for x86")
            || Build.HARDWARE == "goldfish"
            || Build.HARDWARE == "vbox86"
            || Build.HARDWARE.toLowerCase(locale).contains("nox")
            || Build.FINGERPRINT.startsWith("generic")
            || Build.PRODUCT == "sdk"
            || Build.PRODUCT == "google_sdk"
            || Build.PRODUCT == "sdk_x86"
            || Build.PRODUCT == "vbox86p"
            || Build.PRODUCT.toLowerCase(locale).contains("nox")
            || Build.BOARD.toLowerCase(locale).contains("nox")
